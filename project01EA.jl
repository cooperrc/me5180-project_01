...
# ╔═╡ 407e60bb-e1dd-48e9-b3f6-a9285ac99001
begin
    ...
    # Parameters
    h1 = 0.20      # frame height [m]
    w1 = 0.10      # frame width [m]
    L  = 0.15      # pendulum length [m]
    m  = 0.10      # bob mass [kg]
    g  = 9.81      # gravity [m/s^2]

    # Slow and fast spin rates
    Ω_slow = 2.0   # rad/s
    Ω_fast = 12.0  # rad/s
    ...
    function spinning_pendulum!(du, u, p::PendulumParams, t)
        θ  = u[1]
        θ̇ = u[2]
        Ω  = p.Ω
        L  = p.L
    ...
    function simulate(Ω; θ0=0.2, θ̇0=0.0, tspan=(0.0, 10.0), Nt=2001)
        p = PendulumParams(Ω, m, L, g)
        u0 = [θ0, θ̇0]
        prob = ODEProblem(spinning_pendulum!, u0, tspan, p)
        ts = range(tspan[1], tspan[2], length=Nt)
        sol = solve(prob; saveat=ts, reltol=1e-9, abstol=1e-9)
        ...
        # Energies
        T = 0.5 * m * L^2 .* (θ̇.^2 .+ Ω^2 .* sin.(θ).^2)
        V = m * g * L .* (1 .- cos.(θ))
        E = T .+ V

        return (t=t, θ=θ, θ̇=θ̇, x=x, y=y, z=z, T=T, V=V, E=E, Ω=Ω)
    end

    slow = simulate(Ω_slow; tspan=(0.0, 10.0))
    fast = simulate(Ω_fast; tspan=(0.0, 10.0))

    # 1) Angle vs Time (slow vs fast)
    pθ = plot(slow.t, slow.θ;
        label = "θ(t), Ω=$(slow.Ω) rad/s",
        xlabel = "Time (s)", ylabel = "θ (rad)",
        title = "Angle vs Time",
        legend = :topright, framestyle = :box, lw = 2)
    plot!(fast.t, fast.θ; label = "θ(t), Ω=$(fast.Ω) rad/s", lw = 2)

    # 2) Top-View Trajectories (x-y)
    pxy = plot(slow.x, slow.y;
        label = "Ω=$(slow.Ω) rad/s", xlabel = "x (m)", ylabel = "y (m)",
        title = "Top View Trajectory", aspect_ratio = 1, lw = 2,
        legend = :topright, framestyle = :box)

    plot!(fast.x, fast.y; label = "Ω=$(fast.Ω) rad/s", lw = 2, color = :red)

    sqx = (w1/2) .* [-1, 1, 1, -1, -1]
    sqy = (w1/2) .* [-1, -1, 1, 1, -1]
    plot!(sqx, sqy; color = :gray, alpha = 0.4, label = "Frame (top)", ls = :dash)

    pE = plot(slow.t, slow.E; label = "E, Ω=$(slow.Ω)", xlabel = "Time (s)", ylabel = "Energy (J)",
        title = "Total Energy (T+V)", lw = 2, framestyle = :box)
    plot!(fast.t, fast.E; label = "E, Ω=$(fast.Ω)", lw = 2, color = :red)

    # 4) Side-View 2D Animations (x-z), slow & fast
    function sideview_animation(sim; fname::String, fps=60, stride=4)
        anim = @animate for i in 1:stride:length(sim.t)
            x = sim.x[i]
            z = sim.z[i]

            plot([-L, L], [0, 0];
                xlims = (-L, L), ylims = (-max(L,h1), 0.05),
                aspect_ratio = 1, xlabel = "x (m)", ylabel = "z (m)",
                title = "Side View (Ω=$(sim.Ω) rad/s)", legend = false,
                framestyle = :box, alpha = 0.0, size = (600, 450))

            xf = [-w1/2,  w1/2,  w1/2, -w1/2, -w1/2]
            zf = [0.0,    0.0,  -h1,   -h1,    0.0]
            plot!(xf, zf; color = :gray, alpha = 0.5, lw = 1, ls = :dash)

            plot!([0, x], [0, z]; color = :blue, lw = 2)
			
            scatter!([x], [z]; markersize = 6, color = :red)
        end

        gif(anim, fname; fps = fps)
    end

    sideview_animation(slow; fname = "pendulum_side_slow.gif", fps = 60, stride = 4)
    sideview_animation(fast; fname = "pendulum_side_fast.gif", fps = 60, stride = 4)

    # 5) 3D Motion Animations, slow & fast
    function motion3d_animation(sim; fname::String, fps=60, stride=4, cam=(35,30))
        anim = @animate for i in 1:stride:length(sim.t)
            x = sim.x[i]; y = sim.y[i]; z = sim.z[i]
            plot3d([0, 0], [0, 0], [0, 0];
                xlims = (-L, L), ylims = (-L, L), zlims = (-max(L,h1), 0.05),
                xlabel = "x (m)", ylabel = "y (m)", zlabel = "z (m)",
                title = "3D Motion (Ω=$(sim.Ω) rad/s)",
                legend = false, framestyle = :box,
                camera = cam, size = (700, 550), alpha = 0)

            sqx = (w1/2) .* [-1, 1, 1, -1, -1]
            sqy = (w1/2) .* [-1, -1, 1, 1, -1]
            plot3d!(sqx, sqy, zeros(length(sqx)); color = :gray, alpha = 0.4, lw = 1, ls = :dash)
            plot3d!(sqx, sqy, (-h1) .* ones(length(sqx)); color = :gray, alpha = 0.2, lw = 1, ls = :dash)

            trail = 1:i
            plot3d!(sim.x[trail], sim.y[trail], sim.z[trail]; color = :gray, lw = 1.5)

            plot3d!([0, x], [0, y], [0, z]; color = :blue, lw = 2)

            scatter3d!([x], [y], [z]; color = :red, markersize = 4.5)
        end
        gif(anim, fname; fps = fps)
    
    end

    motion3d_animation(slow; fname = "spinning_pendulum_3d_slow.gif", fps = 60, stride = 4, cam = (35,30))
    motion3d_animation(fast; fname = "spinning_pendulum_3d_fast.gif", fps = 60, stride = 4, cam = (35,30))

end

# ╔═╡ 0a25c732-49aa-4b56-bb43-207a90a4a48c
...
# ╔═╡ 4f8dbb0b-e514-4a10-86f1-c63798b765e9
md"""## Lagrangian and Equation of Motion

Let $\theta(t)$ be the bob's angle from the vertical in the **rotating** frame (so the bob lies in the $x'-z'$ plane).  
The **inertial** azimuth is $\phi(t)=\Omega t$ due to the frame's constant rotation.

Bob position in the inertial frame:
\[
x=L\sin\theta\cos(\Omega t),\quad
y=L\sin\theta\sin(\Omega t),\quad
z=-L\cos\theta.
\]

Velocity magnitude (after simplification):
\[
\dot{\mathbf r}^2 = L^2\left(\dot\theta^2 + \Omega^2\sin^2\theta\right).
\]

Kinetic and potential energies:
\[
T=\tfrac{1}{2}mL^2\left(\dot\theta^2+\Omega^2\sin^2\theta\right),\qquad
V=m g L\,(1-\cos\theta).
\]
...
\;\Rightarrow\;
\ddot\theta=\sin\theta\left(\Omega^2\cos\theta-\frac{g}{L}\right).
\]
"""

# ╔═╡ 4b8c2dd8-fe3b-4d41-b38b-41e9e6070284
plot(pθ, pxy, pE; layout = (3,1))
