...
begin
    ...
    # Optional: print clean forms
    # println(latexify(θdd_expr))
    # println(latexify(φdd_expr))
end
...
md"""
## SECTION 5 — Solving the Equations of Motion
...
# ╔═╡ 08b5d495-1519-45b7-8f4b-a832661bf8a4
# Beginning of Cassidy's Code

# ╔═╡ 6145fbc3-d930-4475-9d49-4160b789e205
begin

    @variables t1 m1 L1 g1 w1 r1
    @variables θ1(t1)

    D1 = Differential(t1)

    T1 = (1//2)*m1*( L1^2*(D1(θ1))^2 +
                   w1^2*(r1 + L1*sin(θ1))^2 )
...
# ╔═╡ a19d2123-2ddf-4ed1-a28f-9db6ef4a0d73
begin
	dL_dθdot1 = expand_derivatives(Symbolics.derivative(Lagrange, D1(θ1)))

	dL_dθ1 = expand_derivatives(Symbolics.derivative(Lagrange, θ1))
	
	EL_equation = expand_derivatives(D(dL_dθdot1)-dL_dθ1)

end

# ╔═╡ 70bfaa2f-0889-4cf2-bdb8-a57c77b6677e
begin 
	EL_simplified = simplify(EL_equation /(m1 * L1))
end

# ╔═╡ ffdbe407-7ae2-47c5-aa7d-e523cdd162af
begin
	function rotating_pendulum1!(du1,u1,p1,t1)
	    θ1, θdot1 = u1
	    l1, G1, W1, R1 = p1
	
	    du1[1] = θdot1
	
	    du1[2] = (W1^2/l1)*cos(θ1)*(R1 + l1*sin(θ1)) - (G1/l1)*sin(θ1)
	end
	...
	uθ1 = [0.1, 0.0] #small initial angle
	tspan1 = (0.0, 6.0)
	
	p_slow1 = (l1,G1,w_slow,R1)
	prob_slow1 = ODEProblem(rotating_pendulum1!, uθ1, tspan1, p_slow1)
	sol_slow1 = solve(prob_slow1)
	
	p_fast1 = (l1,G1,w_fast,R1)
	prob_fast1 = ODEProblem(rotating_pendulum1!, uθ1, tspan1, p_fast1)
	sol_fast1 = solve(prob_fast1)
	
	plot(sol_slow1.t, sol_slow1[1,:], label="Slow ω")
	plot!(sol_fast1.t, sol_fast1[1,:], label="Fast ω")
	xlabel!("t")
	ylabel!("θ(t)")
	
end

# ╔═╡ edd0ff3f-d20c-45ca-a251-5f7f95fc3bf2
begin
    gr()
	...
	h1 = 0.2
    number_frames = 200
    time_values = range(sol1.t[1], sol1.t[end], length=number_frames)

    @gif for time_value in time_values

        current_state = sol1(time_value)
        θ1 = current_state[1]
        θdot1 = current_state[2]

        # ----------------------
        # Panel 1: Angle vs Time
        # ----------------------
        p1 = plot(sol1.t, sol1[1,:],
                  xlabel="Time (s)",
                  ylabel="θ (rad)",
                  title="Angle vs Time",
                  legend=false)

        scatter!(p1, [time_value], [θ1])

        # ----------------------
        # Panel 2: Angular Speed
        # ----------------------
        p2 = plot(sol1.t, sol1[2,:],
                  xlabel="Time (s)",
                  ylabel="ω (rad/s)",
                  title="Angular Speed vs Time",
                  legend=false)

        scatter!(p2, [time_value], [θdot1])
        ...
        pivot_x = R1*cos(Ω*time_value)
        pivot_y = R1*sin(Ω*time_value)
		pivot_z = h1
        # Bob position
        bob_x = (R1 + l1*sin(θ1))*cos(Ω*time_value)
        bob_y = (R1 + l1*sin(θ1))*sin(Ω*time_value)
        bob_z = h1 -l1*cos(θ1)
